...
module is the mechanical CI and operator enforcement, shipped in the
wheel as `coldscreen check-language`. scripts/check_language.py is a
thin checkout wrapper with the same argv contract.

The helper strips URLs before matching, so a source URL whose slug
contains a banned word is not a hit; the prose itself remains fully
gated.
...
src/coldscreen/templates/. Those paths are checkout-oriented. An
installed wheel with no checkout may find nothing and exit 0 with the
existing "no target files found" message. Explicit paths work from a
...
on-disk rerun backstop uses. A text that has fewer than two tokens
...
Usage: coldscreen check-language [path ...]
"""
...
from .language import (
    _site_source_label as _site_source_label,
)
from .language import (
...
_evidence_sections = evidence_sections
...
def claim_exemptions(path: Path) -> tuple[str, ...]:
    ...
    Each stored claim text is honored only when claim_quote_is_verified
    against the sibling evidence-section map: substance, a non-empty
    source naming a reconstructed evidence label, and a normalized
    substring of that declared section only. Deck labels are deck p.{page
    key}. Site labels are site {path} from the requested URL path. A
    missing source, unknown label, empty section, or a hit only in a
    different section yields no exemption for that claim. A single-token
    text is dropped even when evidence would re-verify it. Missing or
    unreadable casefile, missing evidence, or a text that fails
    re-verification all yield no exemption for it: the scan runs at full
    strictness, which is the fail-closed direction.
    """
    ...
    if not isinstance(claims, list):
        return ()
    sections = evidence_sections(path.parent / "evidence")
    if not sections:
        return ()
    verified: list[str] = []
    for claim in claims:
        ...
        if (
            ...
            and claim_quote_is_verified(text, source, sections)
        ):
